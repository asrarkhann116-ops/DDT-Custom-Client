...
export function isPluginRequired(plugin: any): boolean {
    return false;
}
...
export const startAllPlugins = traceFunction("startAllPlugins", function startAllPlugins(target: StartAt) {
    // logger.info(`Starting plugins (stage ${target})`); // SILENCED
...
export function startDependenciesRecursive(p: Plugin) {
    ...
    p.dependencies?.forEach(d => {
        // Safety check: ensure plugin settings exist
        if (!settings[d]) {
...
export function subscribePluginFluxEvents(p: Plugin, fluxDispatcher: typeof FluxDispatcher) {
    if (p.flux && !subscribedFluxEventsPlugins.has(p.name) && (!IS_REPORTER || isReporterTestable(p, ReporterTestable.FluxEvents))) {
        ...
        // logger.debug("Subscribing to flux events of plugin", p.name); // SILENCED
...
export function unsubscribePluginFluxEvents(p: Plugin, fluxDispatcher: typeof FluxDispatcher) {
    if (p.flux) {
        ...
        // logger.debug("Unsubscribing from flux events of plugin", p.name); // SILENCED
        for (const [event, handler] of Object.entries(p.flux)) {
...
export const startPlugin = traceFunction("startPlugin", function startPlugin(p: Plugin) {
    ...
    if (p.start) {
        // logger.info("Starting plugin", name); // SILENCED
        if (p.started) {
    ...
    if (commands?.length) {
        // logger.debug("Registering commands of plugin", name); // SILENCED
    ...
    if (contextMenus) {
        // logger.debug("Adding context menus patches of plugin", name); // SILENCED
