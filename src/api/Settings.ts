 ...
 * @returns Settings
 */
export function useSettings(paths?: (UseSettings<Settings> & string)[]) {
    const [, forceUpdate] = React.useReducer(() => ({}), {});

    useEffect(() => {
        if (paths) {
            paths.forEach((p: string | undefined) => {
                if (!p) return;
                if (p.endsWith(".*")) {
            ...
            return () => paths.forEach((p: string | undefined) => {
                if (!p) return;
                if (p.endsWith(".*")) {
...
export function migratePluginToSettings(...args: any[]) {}
export function migrateOldSettingToNewPlugin(...args: any[]) {}
