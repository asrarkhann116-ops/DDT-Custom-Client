...
function DDTTitleBadge() {
    return (
        ...
        >
            <img src={DDT_LOGO_DATA_URL} alt="DDT" className="ddt-titlebar-logo" />
            <span className="ddt-titlebar-text">DDT</span>
        </div>
    );
}
        ...
        >
            {(_, { isShown }) => (
                <HeaderBarButton
                    ...
                    onClick={() => setShow(v => !v)}
                    tooltip={isShown ? null : "DDT Toolbox"}
...
export default definePlugin({
    ...
    TrailingWrapper({ children }: PropsWithChildren) {
        return (
            <>
                {children}
                <ErrorBoundary key="ddt-title-badge" noop>
                    <DDTTitleBadge />
                </ErrorBoundary>
