...
export default definePlugin({
    ...
    async start() {
        ...
        UserProfileStore.getUserProfile = function (userId: string) {
            ...
            // Extract badges early (before any Proxy operations)
            let newBadges = userProfile.badges ? [...userProfile.badges] : [];
            ...
            // Remove duplicates by ID (defensive check)
            const uniqueBadges = Array.from(
                new Map(newBadges.map(badge => [badge.id, badge])).values()
            );
            ...
            // Return original profile with modified badges (avoid creating new object)
            // This way if userProfile is a Proxy, we don't trigger ownKeys trap
            userProfile.badges = uniqueBadges;
            return userProfile;
        };
    },
