...
const PRIVATE_PARTITION_PREFIX = "DDT-private-search";
...
const UBLOCK_ORIGIN_VERSION_PATH = join(UBLOCK_ORIGIN_DIR, ".DDT-release");
const PRELOAD_DIR = join(tmpdir(), "DDT-private-search-browser");
...
async function getLatestUblockOriginAsset(): Promise<UblockOriginAsset> {
    const response = await checkedFetch(UBLOCK_ORIGIN_RELEASES_API, {
        headers: {
            ...
            "User-Agent": `Electron ${process.versions.electron} ~ DDT`
        }
    });
...
function getNavigationScript(homeUrl: string) {
    ...
    return `
        (() => {
            const id = "DDT-private-search-nav";
