...
export function makeDevBanner(state?: string): string | JSX.Element {
    ...
    const format = settings.store.format ?? "{devbannerIcon} {buildChannel} {buildNumber} ({buildHash}) | {DDTIcon} {DDTName} {DDTVersion} ({DDTHash})";
    ...
    const replaced = baseFormat
        ...
        .replace(/{DDTVersion}/g, VERSION)
        .replace(/{DDTHash}/g, gitHashShort)
        .replace(/{DDTPlatform}/g, getVersionInfo(false))
        ...
        .replace(/{DDTHash}/g, clientInfo.shortHash ?? "Not Supported")
        .replace(/{DDTPlatform}/g, `v${clientInfo?.dev ? "Dev Build" : "Standalone"}`)
    ...
    const parts = replaced.split(/({.*?}|__NEWLINE__)/).filter(Boolean).map((part, i) => {
        switch (part) {
            ...
            case "{DDTIcon}":
                return <span key={`icon-DDT-${i}`} className="vc-discord-dev-banner-icons"><DDTIcon /></span>;
            case "{electronIcon}":
