...
import { isDDTGuild, isDDTSupport } from "@utils/misc";
...
export function ChatPluginCard({ url, description }: { url: string, description: string; }) {
    ...
    if (excludedPlugin || !p) {
        const toolTipText = excludedPlugin
            ...
            : "This plugin is not on this version of DDT. Try updating!";
    ...
    if (required) {
        const tooltipText = p.required || !dependents.length
            ? "This plugin is required for DDT to function."
...
export const PluginCards = ErrorBoundary.wrap(function PluginCards({ message }: { message: Message; }) {
    ...
    message.embeds?.forEach(embed => {
        if (!embed.url?.startsWith("https://DDT.org/plugins/") && !embed.url?.startsWith("https://vencord.dev/plugins/")) return;

        const isDDT = isDDTGuild(message.channel_id) && isDDTSupport(message.author.id);
        if (!isDDT) return;
    ...
    if (message.author.id === EQUIBOT_USER_ID && components?.length >= 4) {
        ...
        if (pluginUrl?.startsWith("https://DDT.org/plugins/") || pluginUrl?.startsWith("https://vencord.dev/plugins/")) {
