...
type ColorPickerWithOnCloseProps = {
    onClose?: () => void;
    disabled?: boolean;
...
function ColorPickerWithOnClose(props: ColorPickerWithOnCloseProps): JSX.Element {
    const LiveColorPicker = ColorPicker as ComponentType<any>;

    return <LiveColorPicker {...(props as any)} />;
}
...
export function SettingsColorPicker({
    ...
    return (
        <>
            {label != null && <SettingsParagraph className={withDimmedClass(labelClassName, !!(props as any).disabled)}>{label}</SettingsParagraph>}
            <div className={q("settings-color-picker", className)}>
                <ColorPickerWithOnClose {...(props as any)} />
            </div>
        </>
    );
}
