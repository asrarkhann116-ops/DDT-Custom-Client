# IGP - Plugin di Crittografia PGP per DDT

IGP (DDT GPG/Pgp) è un potente plugin di crittografia per DDT che permette la messaggistica cifrata end-to-end utilizzando la crittografia PGP (Pretty Good Privacy).
...
1. Posiziona la cartella del plugin nella directory `DDTplugins` di DDT:
   ```
   DDT/src/DDTplugins/IGP/
   ```

2. Ricostruisci DDT per includere il plugin

3. Abilita il plugin nelle impostazioni di DDT
...
1. Vai alle impostazioni di DDT
...
IGP è stato sviluppato specificamente per la mod di Discord DDT, estendendo l'architettura dei plugin Vencord. Il plugin sfrutta la libreria OpenPGP.js per fornire solide capacità di crittografia direttamente all'interno di Discord.
...
- **Archiviazione**: Usa l'API DataStore di DDT per la gestione delle chiavi
- **Comandi**: Integrato con il sistema di comandi di DDT
