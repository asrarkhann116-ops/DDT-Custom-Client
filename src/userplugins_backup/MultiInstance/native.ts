...
const DDT_ICON = nativeImage.createFromDataURL(`data:image/png;base64,${DDTIcon}`);
...
export async function openInstance(
    ...
    try {
        const savedPartition = `persist:DDT-mi-${profileId}`;
        ...
        const win = new BrowserWindow({
            ...
            darkTheme: true,
            icon: mode === "detached" ? DDT_ICON : undefined,
            show: false,
            webPreferences,
        });
...
export async function clearSavedSession(
    ...
    try {
        const partition = `persist:DDT-mi-${profileId}`;
