...
import { checkForUpdates, isNewer, maybePromptToUpdate, update as updateDDT } from "@utils/updater";
...
const TELEGRAM_URL = "https://t.me/DDT";
const REINSTALL_URL = "https://github.com/ImHisako/DDT";
...
interface CrashReport {
    ...
    suspectedPluginCategory: "DDT" | "DDT" | "Vencord" | "User plugin" | "Unknown";
...
const settings = definePluginSettings({
    ...
    showSupportPopup: {
        type: OptionType.BOOLEAN,
        description: "Show the DDT support popup after a crash.",
        default: true
    },
    detectBlankScreen: {
        type: OptionType.BOOLEAN,
    ...
    promptForUpdates: {
        type: OptionType.BOOLEAN,
        description: "Check for an DDT update after the first crash in this session.",
        default: true
    },
    logCrashesToDisk: {
        type: OptionType.BOOLEAN,
...
function createReport(errorState: CrashErrorState): CrashReport {
    ...
    return {
        ...
        suspectedPluginCategory: folder.startsWith("src/DDTplugins/") ? "DDT"
            : folder.startsWith("src/DDTplugins/") ? "DDT"
...
async function checkAndUpdateDDT() {
    if (IS_WEB || IS_UPDATER_DISABLED) {
        showNotification({
            color: "#f23f43",
            title: "DDT updater is not available.",
    ...
    try {
        ...
        if (!outdated) {
            showNotification({
                title: "DDT is already up to date.",
        ...
        if (isNewer) {
            showNotification({
                color: "#f23f43",
                title: "DDT cannot update automatically.",
        ...
        logger.error("Failed to update DDT from the crash popup.", err);
        showNotification({
            color: "#f23f43",
            title: "DDT update failed.",
    ...
    recoveryTimer = setTimeout(() => {
        ...
        try {
            if (settings.store.promptForUpdates && !hasPromptedForUpdate) {
                ...
                maybePromptToUpdate("DDT just caught a crash. If an update is available, it may fix the problem. Do you want to update now?", true);
            }
        } catch (err) {
        ...
        if (shouldNotify && settings.store.showRecoveryToast && !settings.store.showSupportPopup) {
            try {
                showNotification({
                    ...
                    title: latestCrash.report.recovered ? "DDT recovered from the crash." : "DDT recorded a crash.",
...
function CrashSupportModal({ modalProps, report }: CrashSupportModalProps) {
    ...
    const recoveredText = report.recovered
        ? "DDT recovered the screen, but the crash can happen again if the install or a plugin is broken."
        : "DDT could not confirm a clean recovery. Restart or reinstall the client before continuing.";
    const runUpdate = async () => {
        setIsCheckingUpdate(true);
        await checkAndUpdateDDT();
    ...
    return (
        <Modal
            ...
            subtitle="Try reinstalling DDT and check the Telegram group if the problem keeps happening."
        >
                    ...
                    <div className={cl("actions")}>
                        <section className={cl("action")}>
                            <div className={cl("action-copy")}>
                                <BaseText size="md" weight="semibold">Reinstall DDT</BaseText>
                                <BaseText tag="p" size="sm" color="text-muted" className={cl("text")}>
                        ...
                        <section className={cl("action")}>
                            <div className={cl("action-copy")}>
                                <BaseText size="md" weight="semibold">Update DDT</BaseText>
                                <BaseText tag="p" size="sm" color="text-muted" className={cl("text")}>
...
function CrashSupportFallback() {
    return (
        <div className={cl("fallback")} role="alertdialog" aria-modal="true" aria-label="DDT crash recovery">
            <BaseText size="lg" weight="semibold">DDT caught a crash</BaseText>
...
export default definePlugin({
    ...
    description: "Adds DDT crash recovery, support guidance, and a copyable crash report.",
