...
function openQuestHome() {
    try {
        ...
        console.error("[DDT Quest] Could not open Discord's quest page:", err);
    }
}
...
// ⚔️ DDT: UpdateNotice removed - getUpdateState() no longer exists in current Vencord build
// function UpdateNotice() removed to prevent crash
    ...
    return (
        <div className="qm-panel">
            <div className="qm-panel-header">
                ...
                <span className="qm-panel-title">DDT Quest Automator</span>
            ...
            {/* ⚔️ DDT: UpdateNotice removed - not needed */}
...
function QuestLauncher() {
    ...
    const label = skipped.length
        ? `DDT Quest: ${skipped.length} skipped`
        : running.length
            ? `DDT Quest: working on ${running.length}`
            : claimable.length
                ? `DDT Quest: ${claimable.length} ready to claim`
                : "DDT Quest";
...
export function unmountQuestPanel() {
    ...
    setTimeout(() => {
        try {
            ...
            console.error("[DDT Quest] Failed to unmount panel:", err);
        }
        container?.remove();
    }, 0);
}
