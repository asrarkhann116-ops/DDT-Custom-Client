            ...
            const armWatchdog = () => {
                ...
                watchdog = setTimeout(() => {
                    console.error(beats === 0
                        ? `[DDT Quest] Discord never sent a heartbeat for "${questName}". It is not accepting the spoofed game on this client, so there is nothing to wait for.`
                        : `[DDT Quest] Discord stopped sending heartbeats for "${questName}" after ${beats}. Giving up.`);
