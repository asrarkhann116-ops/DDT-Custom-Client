...
export function validateIgnoredQuests(qs?: Quest[]): void {
    ...
    const validIgnored = Array.from(new Set<string>(currentlyIgnoredQuests.filter(id => quests.some((quest: any) => quest.id === id) || excludedQuests.some((quest: any) => quest.id === id))));
