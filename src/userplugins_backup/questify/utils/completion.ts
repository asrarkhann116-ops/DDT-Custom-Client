...
export function getQueueableAutoCompleteQuests(): Quest[] {
    ...
    return (Array.from(QuestStore.quests.values()) as any[])
        .filter((quest: any) => !activeAutoCompletes.has(quest.id))
        .filter((quest: any) => questMatchesIncludedTypes(quest, includedTypes))
        .filter(canAutoCompleteQuest)
        .sort((a: any, b: any) => getQuestExpiryTime(a) - getQuestExpiryTime(b));
}
