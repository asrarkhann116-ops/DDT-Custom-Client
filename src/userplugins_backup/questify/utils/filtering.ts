...
export function questMatchesIncludedTypes(quest: Quest, includedTypes: QuestIncludedTypes): boolean {
    const rewardTypeAllowed = quest.config.rewardsConfig.rewards.some((reward: any) => Boolean(includedTypes[reward.type as (QuestTaskType | QuestRewardType)]));
    const taskTypeAllowed = Object.values(quest.config.taskConfigV2.tasks).some((task: any) => Boolean(includedTypes[task.type as (QuestTaskType | QuestRewardType)]));
