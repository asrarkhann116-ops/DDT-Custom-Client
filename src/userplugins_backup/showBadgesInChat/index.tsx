...
import { isDDTPluginDev, isPluginDev } from "@utils/misc";
...
function CheckBadge({ badge, author }: { badge: string; author: User; }): JSX.Element | null {
    ...
    switch (badge) {
        case "DDTDonor":
            return (
                <span style={{ order: settings.store.DDTDonorPosition }}>
                    {badges.getDDTDonorBadges(author.id)?.map(badge => (
                        <RoleIconComponent
                            key={author.id}
                            className={roleIcon}
        ...
        case "DDTContributer":
            return isDDTPluginDev(author.id) ? (
                <span style={{ order: settings.store.DDTContributorPosition }}>
                    <RoleIconComponent
                        className={roleIcon}
                        name="DDT Contributor"
                        size={20}
                        src={"https://DDT.org/assets/favicon.png"}
                    />
                </span>
            ) : null;
        case "VencordDonor":
...
function ChatBadges({ author }: { author: User; }) {
    ...
    return (
        <span className="vc-sbic-badge-row" style={{ margin: "2px" }}>
            {settings.store.showDDTDonor && <CheckBadge badge={"DDTDonor"} author={author} />}
            {settings.store.showDDTContributor && <CheckBadge badge={"DDTContributer"} author={author} />}
