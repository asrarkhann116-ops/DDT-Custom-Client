...
function sourceDiscordVoiceDir(): string | undefined {
    const relativeDiscordVoiceDir = join("src", "DDTplugins", "stereoInstaller.desktop", "StereoMethods", "Discord-Voice");
    const roots = [
        process.cwd(),
        app.getAppPath(),
...
async function discoveredDiscordRoots(): Promise<string[]> {
    ...
    return entries
        .filter((entry: Dirent) => entry.isDirectory() && /(discord|cord|vencord|DDT)/i.test(entry.name))
...
function clientPrefixForRoot(root: string): string {
    const leaf = basename(root).toLowerCase();
    if (["discord", "vencord", "DDT", "bettervencord"].includes(leaf)) return "Stable";
...
function Stop-DiscordUnderRoot {
    ...
    $names = @("Discord.exe", "DiscordCanary.exe", "DiscordPTB.exe", "DiscordDevelopment.exe", "Lightcord.exe", "Vencord.exe", "DDT.exe", "BetterVencord.exe", "Update.exe")
